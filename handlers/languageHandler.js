...
const constants = {
  '%bastion%': 'Zerotwo',
  '%currencyName%': 'Zerotwo Currency',
  '%_currencyName%': 'Zerotwo Currencies',
  '%currencySymbol%': 'ZC',
  '%bastionSite%': 'https://facebook.com/lazySano',
  '%discordInvLink%': 'https://discord.gg/aP2PmBb'
};
...
module.exports = class LanguageHandler {
  ...
  error(locale, key, description = false, ...vars) {
    ...
    if (description) {
      if (!this.locales.get(locale).errors['descriptions'][key]) {
        ...
        return this.error('ar', key, description, ...vars);
      }
  ...
  command(locale, module, command) {
    ...
    if (!this.locales.get(locale).modules[module] || !this.locales.get(locale).modules[module][command]) {
      ...
      return this.command('ar', module, command);
    }
